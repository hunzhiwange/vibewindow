...
pub fn view(app: &App) -> Element<'_, Message> {
    ...
    let backend_row: Element<'_, Message> = field_row(
        "存储后端",
        "选择记忆系统使用的持久化后端。",
        backend_pick,
    );
    ...
    let general_section = column![
            ...
            backend_row,
            settings_divider(),
            bool_row("自动保存", "自动保存用户输入到记忆。", s.auto_save, "自动保存用户输入到记忆", |value| {
                Message::Settings(SettingsMessage::Memory(MemoryMessage::AutoSaveToggled(value)))
            }),
            settings_divider(),
            bool_row("记忆卫生清理", "定期清理低价值或陈旧记忆。", s.hygiene_enabled, "启用记忆卫生清理", |value| {
                Message::Settings(SettingsMessage::Memory(MemoryMessage::HygieneEnabledToggled(
                    value,
                )))
            },),
            settings_divider(),
            bool_row("响应缓存", "缓存部分响应结果以减少重复开销。", s.response_cache_enabled, "启用响应缓存", |value| {
                Message::Settings(SettingsMessage::Memory(
                    MemoryMessage::ResponseCacheEnabledToggled(value),
                ))
            },),
            settings_divider(),
            bool_row("记忆快照", "启用记忆快照导出。", s.snapshot_enabled, "启用记忆快照导出", |value| {
                Message::Settings(SettingsMessage::Memory(MemoryMessage::SnapshotEnabledToggled(
                    value,
                )))
            },),
            settings_divider(),
            ...
            bool_row(
                ...
                "数据库缺失时自动从快照恢复",
                |value| {
                    Message::Settings(SettingsMessage::Memory(MemoryMessage::AutoHydrateToggled(
                        value,
                    )))
                }
            ),
    ...
    let embedding_section = column![
                ...
                row![
                    column![
                        ...
                        row![
                            ...
                            button(text("hint:模式名").size(11))
                                .on_press(Message::Settings(SettingsMessage::SystemTabSelected(SystemTab::EmbeddingRoutes)))
                                .padding([1, 6])
                                .style(|theme: &iced::Theme, _status| {
                                    ...
                                    iced::widget::button::Style {
                                        ...
                                        border: Border { width: 0.0, color: iced::Color::TRANSPARENT, radius: 4.0.into() },
                                        ..Default::default()
                                    }
                                }),
                    ...
                    container(
                        text_input("text-embedding-3-small", &s.embedding_model)
                            .on_input(|value| {
                                Message::Settings(SettingsMessage::Memory(MemoryMessage::EmbeddingModelChanged(
                                    value,
                                )))
                            })
                            .padding(SETTINGS_CONTROL_PADDING)
            ...
            number_row(
                ...
                2,
                |value| {
                    Message::Settings(SettingsMessage::Memory(MemoryMessage::VectorWeightChanged(
                        value,
                    )))
                },
            ),
            ...
            number_row(
                ...
                2,
                |value| {
                    Message::Settings(SettingsMessage::Memory(MemoryMessage::KeywordWeightChanged(
                        value,
                    )))
                },
            ),
    ...
    let qdrant_section = if s.backend == "qdrant" {
        Some(
            column![
                    ...
                    settings_divider(),
                    text_row("Qdrant API 密钥", "可选的鉴权密钥。", "可选", &s.qdrant_api_key_input, |value| {
                        Message::Settings(SettingsMessage::Memory(
