//! SQL 工具视图模块
//!
//! 本模块提供 SQL 美化工具的 UI 视图组件。主要功能包括：
//! - SQL 代码编辑器（支持文本编辑操作）
//! - 自定义滚轮滚动与独立滚动条
//! - SQL 美化、压缩、净化功能按钮
//! - 记忆模式开关（保持用户设置）
...
use crate::app::message::SqlToolMessage;
...
fn build_controls_panel<'a>(app: &'a App) -> Element<'a, Message> {
    let remember_row = row![
        ...
        checkbox(app.sql_tool_remember)
            .on_toggle(|enabled| Message::SqlTool(SqlToolMessage::ToggleRemember(enabled))),
    ]
    .spacing(12)
    .align_y(Alignment::Center);
    ...
    let copy_row = row![
        ...
        button(text("复制").size(13))
            .on_press(Message::SqlTool(SqlToolMessage::Copy))
            .padding([10, 14])
    ...
    let clear_row = row![
        text("清空").size(13).width(Length::Fill),
        button(text("清空").size(13))
            .on_press(Message::SqlTool(SqlToolMessage::Clear))
            .padding([10, 14])
            .style(rounded_action_btn_style),
    ]
    .spacing(12)
    ...
    column![
        ...
        settings_panel(
            column![
                row![
                    build_action_button(app, "美化", SqlToolMessage::Beautify, true),
                    build_action_button(app, "压缩", SqlToolMessage::Compress, false),
                ]
                .spacing(10),
                row![
                    build_action_button(app, "净化", SqlToolMessage::Purify, false),
                    Space::new().width(Length::Fill),
                ]
                .spacing(10),
            ]
        ...
        settings_panel(column![remember_row, copy_row, clear_row].spacing(14)),
    ]
    .spacing(12)
    .width(Length::Fill)
    .into()
...
fn build_action_button<'a>(
    ...
    let button = if app.sql_tool_loading { button } else { button.on_press(Message::SqlTool(msg)) };
        ...
        settings_panel(
            column![
                row![
                    ...
                    build_metric_badge(format!("{} 行", app.sql_tool_editor.line_count().max(1))),
                ]
                .align_y(Alignment::Center)
                .spacing(12),
                editor_panel,
...
fn build_status_badge<'a>(app: &'a App) -> Element<'a, Message> {
    ...
    let (label, tone): (String, StatusTone) = if app.sql_tool_loading {
        ("处理中".to_string(), StatusTone::Loading)
    } else if let Some(message) = &app.sql_tool_notification {
...
fn build_editor_panel<'a>(app: &'a App, size: Size) -> Element<'a, Message> {
    let editor = text_editor(&app.sql_tool_editor)
        .id(app.sql_tool_editor_id.clone())
        .placeholder("输入 SQL")
        .on_action(|action| Message::SqlTool(SqlToolMessage::EditorAction(action)))
        .height(Length::Fill)
    ...
    let editor = wrap_with_context_menu(
        ...
        TextEditorContextMenuState {
            open: app.sql_tool_context_menu_open,
            position: app.sql_tool_context_menu_pos,
        },
        |point| Message::SqlTool(SqlToolMessage::OpenContextMenu { x: point.x, y: point.y }),
        TextEditorContextMenuMessages {
            close: Message::SqlTool(SqlToolMessage::CloseContextMenu),
            copy: Message::SqlTool(SqlToolMessage::ContextMenuCopy),
            cut: Message::SqlTool(SqlToolMessage::ContextMenuCut),
            paste: Message::SqlTool(SqlToolMessage::ContextMenuPaste),
            delete: Message::SqlTool(SqlToolMessage::ContextMenuDelete),
        },
    );
    ...
    text_editor_scroll_panel(
        ...
        TextEditorScrollPanelMetrics {
            ...
            line_count: app.sql_tool_editor.line_count(),
            scroll_top_line: app.sql_tool_scroll_top_line,
        },
        |delta, viewport_height| {
            Message::SqlTool(SqlToolMessage::EditorWheelScrolled { delta, viewport_height })
        },
        |top_line, viewport_height| {
            Message::SqlTool(SqlToolMessage::ScrollbarChanged { top_line, viewport_height })
        },
    )
}
