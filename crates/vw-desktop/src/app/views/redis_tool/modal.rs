    ...
    let count_stepper = row![
        button(text("-").size(18))
            .on_press_maybe((!is_busy).then_some(Message::RedisTool(
                RedisToolMessage::DecreaseDefaultLoadCount,
            )))
            .padding([8, 12])
        ...
        button(text("+").size(18))
            .on_press_maybe((!is_busy).then_some(Message::RedisTool(
                RedisToolMessage::IncreaseDefaultLoadCount,
            )))
            .padding([8, 12])
            .style(rounded_action_btn_style),
        button(text(if is_busy { "保存中..." } else { "保存" }).size(13))
            .on_press_maybe((!is_busy).then_some(Message::RedisTool(
                RedisToolMessage::SaveDefaultLoadCount,
            )))
            .padding([10, 14])
            ...
            column![
                ...
                row![
                    button(text(if is_busy { "处理中..." } else { "导出" }).size(13))
                        .on_press_maybe((!is_busy).then_some(Message::RedisTool(
                            RedisToolMessage::ExportConfigs,
                        )))
                        .padding([10, 14])
                        .style(rounded_action_btn_style),
                    button(text(if is_busy { "处理中..." } else { "导入" }).size(13))
                        .on_press_maybe((!is_busy).then_some(Message::RedisTool(
                            RedisToolMessage::ImportConfigs,
                        )))
                        .padding([10, 14])
                        .style(primary_action_btn_style),
                ]
    ...
    let content = column![
        ...
        row![
            ...
            button(text("上一页").size(12))
                .on_press_maybe((!app.redis_tool.is_gateway_loading() && app.redis_tool.history_page_offset > 0)
                    .then_some(Message::RedisTool(RedisToolMessage::HistoryPreviousPage)))
                .padding([8, 12])
                .style(rounded_action_btn_style),
            button(text("下一页").size(12))
                .on_press_maybe((!app.redis_tool.is_gateway_loading() && app.redis_tool.history_has_more)
    ...
    let content = column![
            ...
            column![
                form_row(
                    ...
                    text_input("例如：cache:user:1", &app.redis_tool.create_key_draft.name)
                        .on_input(|value| Message::RedisTool(RedisToolMessage::CreateKeyNameChanged(value)))
                        .padding([10, 12])
                        .size(13)
                        .width(Length::Fill)
                        .style(settings_text_input_style)
        ...
        row![
            ...
            button(text("取消").size(13))
                .on_press_maybe((!is_busy).then_some(Message::RedisTool(
                    RedisToolMessage::CloseCreateKeyModal,
                )))
                .padding([10, 14])
                .style(rounded_action_btn_style),
            button(text(if is_busy { "创建中..." } else { "创建" }).size(13))
                .on_press_maybe(can_submit.then_some(Message::RedisTool(
                    RedisToolMessage::ConfirmCreateKey,
                )))
                .padding([10, 14])
                .style(primary_action_btn_style),
        ]
