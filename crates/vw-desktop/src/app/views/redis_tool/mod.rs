...
use modal::{build_create_key_modal, build_history_modal, build_settings_modal};
    ...
    let hero = container(
        row![
            column![
                ...
                iced::widget::text("连接管理、导入导出与高级连接模板工作台")
                    .size(12)
                    .style(crate::app::components::system_settings_common::settings_muted_text_style),
            ]
            .spacing(4),
...
fn build_workspace<'a>(app: &'a App, size: Size) -> Element<'a, Message> {
    ...
    if size.width >= 980.0 {
        ...
        column![
            container(sidebar).width(Length::Fill),
            container(detail).width(Length::Fill).height(Length::Fill),
        ]
        .spacing(16)
        .width(Length::Fill)
