...
fn remap_provider_options_moves_matching_key() {
    let remapped = remap_provider_options(&json!({"openai": {"temperature": 0.2}, "keep": true}), "openai", "azure");
...
fn provider_options_normalizes_adapter_aliases() {
    let options = provider_options("openai", "openai-compatible", json!({"openai": {"maxTokens": 12}}));
