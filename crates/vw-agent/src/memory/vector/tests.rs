//! 向量相似度、字节编码和混合检索合并策略的回归测试。
//!
//! 本文件验证向量工具在异常浮点值、维度不匹配、重复结果和归一化边界下保持确定性，
//! 避免检索排序因输入形态变化而产生不可解释的结果。
...
mod tests {
    ...
    fn hybrid_merge_deduplicates() {
        ...
        // 同一记忆命中两种检索来源时应合并分数，而不是向用户返回重复条目。
    ...
    fn hybrid_merge_respects_limit() {
        let vec_results: Vec<(String, f32)> =
            (0..20).map(|i| (format!("item_{i}"), 1.0 - i as f32 * 0.05)).collect();
    ...
    fn cosine_nan_returns_zero() {
        ...
        let b = vec![1.0, 2.0, 3.0];
        let sim = cosine_similarity(&a, &b);
        // 外部嵌入服务可能返回异常浮点值；相似度必须保持有限值，避免污染排序。
    ...
    fn bytes_to_vec_non_aligned_truncates() {
        ...
        // 旧数据或截断写入可能留下非 4 字节对齐的尾部，解析时只保留完整 f32。
