...
/// and forwards PRIVMSG messages to the `VibeWindow` message bus.
...
fn split_message(message: &str, max_bytes: usize) -> Vec<String> {
    ...
    if max_bytes == 0 {
        ...
        for l in message.lines().map(|l| l.trim_end_matches('\r')).filter(|l| !l.is_empty()) {
            if !full.is_empty() {
                full.push(' ');
            }
            full.push_str(l);
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(&mut writer, &format!("USER {} 0 * :VibeWindow", self.username)).await?;
        ...
        loop {
            ...
            match msg.command.as_str() {
                ...
                "PRIVMSG" => {
                    ...
                    let reply_target =
                        if is_channel { target.to_string() } else { sender_nick.to_string() };
