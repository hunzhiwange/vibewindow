...
//! session_path = "~/.vibewindow/whatsapp-session.db"  # Required for Web mode
...
fn parse_wa_attachment_markers(message: &str) -> (String, Vec<WaAttachment>) {
    ...
    while cursor < message.len() {
        ...
        let parsed = marker.split_once(':').and_then(|(kind, target)| {
            ...
            Some(WaAttachment { kind, target: target.to_string() })
        });
...
/// ```toml
/// [channels_config.whatsapp]
/// session_path = "~/.vibewindow/whatsapp-session.db"
...
impl WhatsAppWebChannel {
    ...
    fn render_pairing_qr(code: &str) -> Result<String> {
        ...
        Ok(qr.render::<qrcode::render::unicode::Dense1x2>().quiet_zone(true).build())
    }
    ...
    async fn send_media_attachment(
        ...
        tracing::info!("WhatsApp Web: sent {:?} media (id: {})", attachment.kind, msg_id);
        Ok(())
    }
}
...
impl Channel for WhatsAppWebChannel {
    ...
    async fn send(&self, message: &SendMessage) -> Result<()> {
        ...
        if !text_without_markers.is_empty() {
            ...
            tracing::debug!("WhatsApp Web: sent text to {} (id: {})", message.recipient, msg_id);
        }
