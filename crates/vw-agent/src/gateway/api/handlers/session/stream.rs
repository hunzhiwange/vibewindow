...
pub(super) async fn chat_stream(
    ...
    let sse = with_instance(dir.clone(), move || {
        Box::pin(async move {
            ...
            let resume_history_only = options
                .get("desktop_resume_history_only")
            ...
            std::thread::spawn(move || {
                ...
                for (idx, message) in history_messages.into_iter().enumerate() {
                    ...
                    let normalized_content = if role == "tool" {
                        tool_content_to_session_text(&content)
                    } else {
                        content
                    };

                    if idx + 1 == total_messages && role == "user" {
                        // 最后一条 user 消息作为本轮 query，其余消息进入上下文历史。
                ...
                {
                    // 某些客户端会把所有消息都放进 history；这里兜底抽出最后一条用户消息作为 query。
                ...
                let (approval_prompt_tx, _approval_prompt_rx) =
                ...
                agent_session::processor::run(
                    ...
                    move |ev| {
                        ...
                        tx.send(Ok(Bytes::from(frame))).is_ok()
                    },
                );
            });
