...
当你修复一个由无效数据引起的 bug 时，在一个地方添加验证感觉就足够了。但那个单一检查可以被不同的代码路径、重构或模拟绕过。
...
async function gitInit(directory: string) {
  ...
  if (process.env.NODE_ENV === 'test') {
    ...
    if (!normalized.startsWith(tmpDir)) {
      throw new Error(
        `拒绝在测试期间于临时目录外执行 git init: ${directory}`
      );
    }
  }
  // ... 继续
...
- 第三层：`WorktreeManager` 在测试中拒绝在临时目录外执行 git init
