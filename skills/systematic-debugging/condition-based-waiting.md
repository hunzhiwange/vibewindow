...
不稳定的测试通常通过任意延迟来猜测时间。这会创建竞态条件，测试在快速机器上通过但在高负载或 CI 环境中失败。
...
digraph when_to_use {
    ...
    "在测试计时行为？" [shape=diamond];
    "记录为什么需要超时" [shape=box];
    "使用基于条件的等待" [shape=box];

    "测试使用了 setTimeout/sleep？" -> "在测试计时行为？" [label="是"];
    "在测试计时行为？" -> "记录为什么需要超时" [label="是"];
    "在测试计时行为？" -> "使用基于条件的等待" [label="否"];
}
```
...
// 200ms = 100ms 间隔的 2 次触发 - 有文档记录和理由
```
