...
usage() {
  cat <<'USAGE'
VibeWindow installer bootstrap engine

Usage:
  ./vibewindow_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds VibeWindow only (requires existing Rust toolchain).
...
Options:
  ...
  --docker-reset             Reset existing VibeWindow Docker containers/networks/volumes and data dir before --docker bootstrap
  ...
  --build-first              Alias for explicitly enabling separate `cargo build --release --locked -p vw-cli --bin vibewindow --bin vibe-agent`
  --skip-build               Skip build step (`cargo build --release --locked -p vw-cli --bin vibewindow --bin vibe-agent` or Docker image build)
  --skip-install             Skip `cargo install --path crates/vw-cli --force --locked --bin vibewindow --bin vibe-agent`
  -h, --help                 Show help

Examples:
  ./vibewindow_install.sh
  ./vibewindow_install.sh --guided
  ./vibewindow_install.sh --install-system-deps --install-rust
  ./vibewindow_install.sh --prefer-prebuilt
  ./vibewindow_install.sh --prebuilt-only
  ./vibewindow_install.sh --docker --docker-config ./config.toml --docker-daemon
...
Environment:
  ...
  VIBEWINDOW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  VIBEWINDOW_DOCKER_IMAGE      Docker image tag to build/run (default: vibewindow-bootstrap:local)
  VIBEWINDOW_DOCKER_BROWSER_RUNTIME
                            Browser runtime provisioning mode for --docker: "auto" (prompt), "on", or "off"
  VIBEWINDOW_DOCKER_BROWSER_SIDECAR_IMAGE
                            Browser WebDriver sidecar image (default: selenium/standalone-chromium:latest)
  VIBEWINDOW_DOCKER_BROWSER_SIDECAR_NAME
                            Browser WebDriver sidecar container name (default: vibewindow-browser-webdriver)
  VIBEWINDOW_DOCKER_NETWORK    Docker network for VibeWindow + sidecars (default: vibewindow-bootstrap-net)
  VIBEWINDOW_DOCKER_CARGO_FEATURES
                            Extra Cargo features for Docker builds (comma-separated)
  VIBEWINDOW_CARGO_FEATURES    Extra Cargo features for local source builds (comma-separated)
  VIBEWINDOW_CONFIG_PATH       Config path used for channel feature auto-detection (default: ~/.vibewindow/config.toml)
  VIBEWINDOW_DOCKER_DAEMON_NAME
                            Daemon container name for --docker-daemon (default: vibewindow-daemon)
  VIBEWINDOW_DOCKER_DAEMON_BIND_HOST
                            Host bind address for daemon port publish (default: 127.0.0.1)
  VIBEWINDOW_DOCKER_DAEMON_HOST_PORT
                            Host port to publish daemon gateway (default: same as gateway.port)
  VIBEWINDOW_DOCKER_SECRET_KEY_FILE
                            Host path to .secret_key used when seeding encrypted config.toml
  VIBEWINDOW_API_KEY           Used when --api-key is not provided
  VIBEWINDOW_PROVIDER          Used when --provider is not provided (default: openrouter)
  VIBEWINDOW_MODEL             Used when --model is not provided
  VIBEWINDOW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  VIBEWINDOW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  VIBEWINDOW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${VIBEWINDOW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${VIBEWINDOW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/hunzhiwange/vibewindow/releases/latest/download/vibewindow-${target}.tar.gz"
  temp_dir="$(mktemp -d -t vibewindow-prebuilt-XXXXXX)"
  archive_path="$temp_dir/vibewindow-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/vibewindow"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name vibewindow -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable vibewindow binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/vibewindow"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/vibewindow"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/vibewindow-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  echo
  echo "VibeWindow guided installer"
  ...
  if prompt_yes_no "Install vibewindow into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${VIBEWINDOW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set VIBEWINDOW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set VIBEWINDOW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
is_vibewindow_container() {
  ...
  [[ "$name_lc" == *"vibewindow"* || "$image_lc" == *"vibewindow"* || "$command_lc" == *"vibewindow"* ]]
}

is_vibewindow_resource_name() {
  local name="$1"
  local name_lc
  name_lc="$(printf '%s' "$name" | tr '[:upper:]' '[:lower:]')"
  [[ "$name_lc" == *"vibewindow"* ]]
...
maybe_stop_running_vibewindow_containers() {
  ...
  while IFS=$'\t' read -r id name image command; do
    ...
    if is_vibewindow_container "$name" "$image" "$command"; then
      running_ids+=("$id")
  ...
  warn "Detected running VibeWindow container(s):"
  ...
  if ! guided_input_stream >/dev/null 2>&1; then
    warn "Non-interactive mode detected; leaving existing VibeWindow containers running."
    return 0
  fi

  if prompt_yes_no "Stop these running VibeWindow containers before continuing?" "yes"; then
    info "Stopping ${#running_ids[@]} VibeWindow container(s)"
    "$CONTAINER_CLI" stop "${running_ids[@]}" >/dev/null
  else
    warn "Continuing with existing VibeWindow containers still running."
  fi
}

reset_docker_vibewindow_resources() {
  ...
  info "Resetting VibeWindow Docker resources"
  ...
  if [[ ${#container_ids[@]} -gt 0 ]]; then
    info "Removing ${#container_ids[@]} VibeWindow container(s)"
    ...
    info "No existing VibeWindow containers found"
  fi
  ...
  while IFS= read -r resource_name; do
    ...
    if is_vibewindow_resource_name "$resource_name"; then
      network_names+=("$resource_name")
    fi
  done < <("$CONTAINER_CLI" network ls --format '{{.Name}}')

  if [[ ${#network_names[@]} -gt 0 ]]; then
    info "Removing ${#network_names[@]} VibeWindow network(s)"
    ...
    info "No existing VibeWindow networks found"
  fi
  ...
  if [[ ${#volume_names[@]} -gt 0 ]]; then
    info "Removing ${#volume_names[@]} VibeWindow volume(s)"
    ...
    info "No existing VibeWindow volumes found"
  fi
...
run_docker_bootstrap() {
  ...
  docker_browser_sidecar_image="${VIBEWINDOW_DOCKER_BROWSER_SIDECAR_IMAGE:-selenium/standalone-chromium:latest}"
  docker_network="${VIBEWINDOW_DOCKER_NETWORK:-vibewindow-bootstrap-net}"
  docker_daemon_name="${VIBEWINDOW_DOCKER_DAEMON_NAME:-vibewindow-daemon}"
  docker_daemon_bind_host="${VIBEWINDOW_DOCKER_DAEMON_BIND_HOST:-127.0.0.1}"
  docker_daemon_host_port="${VIBEWINDOW_DOCKER_DAEMON_HOST_PORT:-}"
  seed_config_path="${DOCKER_CONFIG_FILE:-}"
  seed_secret_key_path="${DOCKER_SECRET_KEY_FILE:-${VIBEWINDOW_DOCKER_SECRET_KEY_FILE:-}}"
  container_network_name=""
  docker_browser_webdriver_url=""
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.vibewindow-docker"
  else
    default_data_dir="$WORK_DIR/.vibewindow-docker"
  fi
  docker_data_dir="${VIBEWINDOW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  if [[ "$DOCKER_RESET" == true ]]; then
    reset_docker_vibewindow_resources "$docker_data_dir"
  fi

  mkdir -p "$docker_data_dir/.vibewindow" "$docker_data_dir/workspace"
  ...
  if [[ -n "$seed_config_path" ]]; then
    ...
    install -m 600 "$seed_config_path" "$docker_data_dir/.vibewindow/config.toml"
    seed_docker_secret_key_for_config "$seed_config_path" "$docker_data_dir/.vibewindow" "$seed_secret_key_path"
  fi
  ...
  case "$(printf '%s' "$docker_browser_runtime_mode" | tr '[:upper:]' '[:lower:]')" in
    ...
    *)
      ...
      if [[ "$docker_browser_runtime_bool" == "invalid" ]]; then
        warn "Invalid VIBEWINDOW_DOCKER_BROWSER_RUNTIME='$docker_browser_runtime_mode' (expected auto/on/off). Defaulting to off."
        docker_browser_runtime_bool="false"
      fi
      ;;
  esac
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if [[ -n "$docker_build_features" ]]; then
      ...
      docker_build_args+=(--build-arg "VIBEWINDOW_CARGO_FEATURES=$docker_build_features")
    fi
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official VibeWindow image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.vibewindow:/vibewindow-data/.vibewindow"
  workspace_mount="$docker_data_dir/workspace:/vibewindow-data/workspace"
  ...
  if [[ -n "$docker_browser_webdriver_url" ]]; then
    container_extra_env_args+=(-e "VIBEWINDOW_DOCKER_WEBDRIVER_URL=$docker_browser_webdriver_url")
  fi
  ...
  if [[ "$DOCKER_DAEMON_MODE" == true ]]; then
    ...
    if [[ -f "$docker_data_dir/.vibewindow/config.toml" ]]; then
      config_gateway_port_value="$(config_gateway_port "$docker_data_dir/.vibewindow/config.toml" || true)"
    fi
    ...
    daemon_cmd+=(
      ...
      -e HOME=/vibewindow-data
      -e VIBEWINDOW_WORKSPACE=/vibewindow-data/workspace
      -e VIBEWINDOW_DOCKER_BOOTSTRAP=1
    )
...
Use either:
  --api-key "sk-..."
or:
  VIBEWINDOW_API_KEY="sk-..." ./vibewindow_install.sh --docker
or run interactive:
  ./vibewindow_install.sh --docker --interactive-onboard
MSG
        exit 1
      fi
      if [[ -n "$MODEL" ]]; then
...
CONTAINER_CLI="${VIBEWINDOW_CONTAINER_CLI:-docker}"
API_KEY="${VIBEWINDOW_API_KEY:-}"
PROVIDER="${VIBEWINDOW_PROVIDER:-openrouter}"
MODEL="${VIBEWINDOW_MODEL:-}"
LOCAL_CARGO_FEATURES="${VIBEWINDOW_CARGO_FEATURES:-}"
LOCAL_CONFIG_PATH="${VIBEWINDOW_CONFIG_PATH:-$HOME/.vibewindow/config.toml}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${VIBEWINDOW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set VIBEWINDOW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t vibewindow-bootstrap-XXXXXX)"
...
if [[ "$DOCKER_MODE" == true ]]; then
  if [[ -n "$LOCAL_CARGO_FEATURES" ]]; then
    warn "--cargo-features / VIBEWINDOW_CARGO_FEATURES are ignored with --docker (use VIBEWINDOW_DOCKER_CARGO_FEATURES)."
  fi
  ensure_docker_ready
  ...
  echo
  echo "Your containerized VibeWindow data is persisted under:"
  echo "  $DOCKER_DATA_DIR"
  echo

  if [[ "$DOCKER_DAEMON_MODE" == true ]]; then
    daemon_name="${VIBEWINDOW_DOCKER_DAEMON_NAME:-vibewindow-daemon}"
    ...
    echo "  ./vibewindow_install.sh --docker --interactive-onboard"
  elif [[ "$RUN_ONBOARD" == false ]]; then
    echo "Onboarding was intentionally skipped (pre-seeded config mode)."
    echo
    echo "Next steps:"
    echo "  ./vibewindow_install.sh --docker --docker-config ./config.toml --docker-daemon"
    echo "  ./vibewindow_install.sh --docker --interactive-onboard"
  else
    cat <<'DONE'
Next steps:
  ./vibewindow_install.sh --docker --interactive-onboard
  ./vibewindow_install.sh --docker --api-key "sk-..." --provider openrouter
  ./vibewindow_install.sh --docker --docker-config ./config.toml --docker-daemon
DONE
  fi
  exit 0
...
or re-run with:
  ./vibewindow_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_BUILD" == false ]]; then
  ...
  BUILD_CMD=(cargo build --release --locked -p vw-cli --bin vibewindow --bin vibe-agent)
  if [[ -n "$LOCAL_CARGO_FEATURES" ]]; then
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing vibewindow to cargo bin"
  INSTALL_CMD=(cargo install --path "$WORK_DIR/crates/vw-cli" --force --locked --bin vibewindow --bin vibe-agent)
  if [[ -n "$LOCAL_CARGO_FEATURES" ]]; then
...
VIBEWINDOW_BIN=""
if have_cmd vibewindow; then
  VIBEWINDOW_BIN="vibewindow"
elif [[ -x "$HOME/.cargo/bin/vibewindow" ]]; then
  VIBEWINDOW_BIN="$HOME/.cargo/bin/vibewindow"
elif [[ -x "$WORK_DIR/target/release/vibewindow" ]]; then
  VIBEWINDOW_BIN="$WORK_DIR/target/release/vibewindow"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$VIBEWINDOW_BIN" ]]; then
    error "onboarding requested but vibewindow binary is not available."
    error "Run without --skip-install, or ensure vibewindow is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$VIBEWINDOW_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  VIBEWINDOW_API_KEY="sk-..." ./vibewindow_install.sh --onboard
or run interactive:
  ./vibewindow_install.sh --interactive-onboard
MSG
      exit 1
    fi
    if [[ -n "$MODEL" ]]; then
    ...
    ONBOARD_CMD=("$VIBEWINDOW_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
...
Next steps:
  vibewindow status
  vibewindow agent -m "Hello, VibeWindow!"
  vibewindow gateway
DONE
