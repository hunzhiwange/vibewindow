...
INSTALLER_LOCAL="$(cd "$SCRIPT_DIR/.." >/dev/null 2>&1 && pwd || pwd)/vibewindow_install.sh"
BOOTSTRAP_LOCAL="$SCRIPT_DIR/bootstrap.sh"
REPO_URL="https://github.com/hunzhiwange/vibewindow.git"

echo "[deprecated] scripts/install.sh -> ./vibewindow_install.sh" >&2
...
TEMP_DIR="$(mktemp -d -t vibewindow-install-XXXXXX)"
cleanup() {
  rm -rf "$TEMP_DIR"
}
trap cleanup EXIT
...
if [[ -x "$TEMP_DIR/vibewindow_install.sh" ]]; then
  exec "$TEMP_DIR/vibewindow_install.sh" "$@"
fi
...
echo "error: vibewindow_install.sh/bootstrap.sh was not found in the fetched revision." >&2
echo "Run the local bootstrap directly when possible:" >&2
echo "  ./vibewindow_install.sh --help" >&2
exit 1
